...
function change(){
    ...
    //calculate time
    roomSize=document.getElementById("roomsize").value.toString().split('*');
    h=roomSize[0];
    w=roomSize[1];
    sampleD=document.getElementById("sampledistance").value;
    ct=document.getElementById("count").value;
    document.getElementById("approximate_time").innerHTML=Number((h/sampleD+1)*(w/sampleD+1)*ct*(10**(-4))).toFixed(2);
    console.log(document.getElementById("approximate_time").innerHTML);
    }
...
function validInput(){
    ...
    const url = "http://localhost:443/path?"+input;//local
    // const url = "http://indoorloc-sim.cc.gatech.edu:443/path?"+input;//VM
